...
public class SetupManager {
    ...
    public static String getBaseDirectory() {
        if (HostManager.isVirtualHostEnabled()) {
            ...
            LogUtil.info(SetupManager.class.getName(), "Virtual host support enabled");
            LogUtil.info(SetupManager.class.getName(), "Base Directory "+baseDirectory);
...
//    	String basePath = SetupManager.getSettingValue("dataFileBasePath");
//        return basePath != null ? basePath : BASE_DIRECTORY;
    ...
    public static void refreshCache() {
        if (cache != null) {
            synchronized(cache) {
                String profile = DynamicDataSourceManager.getCurrentProfile();
                LogUtil.debug(SetupManager.class.getName(), "Refreshing setup cache for " + profile);
                cache.remove(profile);
	...
	public static Setting getSettingByProperty(String property) {
        if (cache != null) {
    ...
    public void setSetupDao(SetupDao setupDao) {
        SetupManager.setupDao = setupDao;
    }
}
