...
public class UndoHistoryManagerImpl implements UndoHistoryManager {
	...
	public void registerEvents(@SuppressWarnings("rawtypes") List xpdlInfoList, XPDLElementChangeInfo selectedEvent) {
		if (max <= 0) {
			return;
		}
	...
	public void undo() {
		if (!undo.isEmpty()) {
			...
			XPDLElementChangeInfo lastEvent = (XPDLElementChangeInfo) undoChangedList
					.remove(undoChangedList.size() - 1);
	...
	public void redo() {
		if (!redo.isEmpty()) {
			...
			XPDLElementChangeInfo lastEvent = (XPDLElementChangeInfo) redoChangedList
					.remove(redoChangedList.size() - 1);
	...
	protected List revertChangedList(List lst) {
		...
		for (int i = lst.size() - 1; i >= 0; i--) {
			...
			} else if (chNew.getAction() == XPDLElementChangeInfo.UPDATED
					|| chNew.getAction() == XPDLElementChangeInfo.REPOSITIONED) {
