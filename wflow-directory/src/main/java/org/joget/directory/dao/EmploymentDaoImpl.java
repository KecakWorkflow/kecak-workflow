...
public class EmploymentDaoImpl extends AbstractSpringDao implements EmploymentDao {
	...
	public Boolean addEmployment(Employment employment) {
		try {
			...
			employment.setCreatedBy(currentUsername);
			employment.setModifiedBy(currentUsername);
			employment.setDateCreated(currentDate);
			employment.setDateModified(currentDate);
	...
	@SuppressWarnings("unchecked")
	public Collection<Employment> findEmployments(String condition, Object[] params, String sort, Boolean desc,
	...
	public Boolean assignUserAsDepartmentHOD(String userId, String departmentId) {
		try {
			...
			if (user != null && user.getEmployments() != null && user.getEmployments().size() > 0 && department != null) {
	        	...
	        	employment.setModifiedBy(currentUsername);
	        	employment.setDateModified(new Date());
	...
	public Boolean assignUserReportTo(String userId, String reportToUserId) {
		try {
			...
			// get only 1st employment
			if (user != null && user.getEmployments() != null && user.getEmployments().size() > 0
					&& reportToUser != null && reportToUser.getEmployments() != null
					&& reportToUser.getEmployments().size() > 0) {
				Employment employment = (Employment) user.getEmployments().iterator().next();
	...
	@SuppressWarnings("unchecked")
	public Collection<Employment> getEmploymentsNoHaveOrganization(String filterString, String sort, Boolean desc,
			Integer start, Integer rows) {
		try {
			if (filterString == null) {
				filterString = "";
	...
	@SuppressWarnings("unchecked")
	public Collection<Employment> getEmploymentsNotInDepartment(String filterString, String organizationId,
			String departmentId, String sort, Boolean desc, Integer start, Integer rows) {
		try {
			if (filterString == null) {
				filterString = "";
			}
	...
	@SuppressWarnings("unchecked")
	public Collection<Employment> getEmploymentsNotInGrade(String filterString, String organizationId, String gradeId,
